...
function questCardMarkup(offer) {
  ...
  return `<article class="quest-card tarot ${offer.selected ? 'selected' : ''}">
    <div class="tarot-frame">
      ...
      <div class="tarot-kicker"><span>${questRoman(offer.slot)}</span><span>${escapeHtml(q.category || 'quest')}</span></div>
      <div class="quest-art tarot-art">
        <img src="${questArt(q, offer.slot)}" alt="Arte da carta ${escapeHtml(q.title)}" loading="eager" onerror="this.style.display='none';this.parentElement.classList.add('image-failed')">
      </div>
  ...
  $('#activeQuestMini').innerHTML = `<div class="section-head compact"><div><p class="eyebrow">QUEST DA SEMANA</p><h3>${escapeHtml(q.title)}</h3></div><button class="chip" data-go-view="quests">trocar</button></div>
    <div class="mini-quest tarot-mini">
      <div class="mini-tarot-card">
        <span class="mini-tarot-kicker">${questRoman(selected.slot)}</span>
        <img src="${questArt(q, selected.slot)}" alt="${escapeHtml(q.title)}" onerror="this.style.display='none'">
